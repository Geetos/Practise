...
import com.jxtele.projectmanage.dao.RoleDao;
...
import com.jxtele.projectmanage.util.*;
...
//	private RedisTemplate<String, Object> redis;
	...
	public HashMap<String, Object> login(ParameterMap pm, HttpSession session) {
		...
		try {
			...
			Integer level = 0;
			for(Iterator it = uRoles.iterator();it.hasNext();){
				Role role = (Role)it.next();
				if(role.getUsercontrolId() != null){
					Integer _level = userControlMapper.selectByPrimaryKey(role.getUsercontrolId()).getControlLevel();
					list.add(_level);
				}
			...
			if(list != null&&!list.isEmpty()) {
				...
				throw new ClassCastException("请设置用户等级");
			}
			long maxMenuId = menuService.getMaxId();
			UserRole uRole = new UserRole(new BigInteger("0"),new BigInteger("0"),new BigInteger("0"),new BigInteger("0"),new BigInteger("0"),level);
			checkUserRole(uRoles, uRole, maxMenuId);
			uRole.setLevel(list.get(list.size() - 1));
