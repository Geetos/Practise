...
import com.jxtele.projectmanage.entity.UserRole;
			...
			for(int j=0;j<menuList.get(i).getSubMenu().size();j++){
				if(menuUrl.equals(menuList.get(i).getSubMenu().get(j).getMENU_URL())){
					...
					if(!menuList.get(i).getSubMenu().get(j).isHasMenu()){				
						...
						levelmap.put("level",userRole.getLevel());
						session.removeAttribute(Const.SESSION_QX);
						session.removeAttribute(Const.SESSION_LEVEL);
						//重新分配按钮权限
						session.setAttribute(Const.SESSION_QX, map);
						session.setAttribute(Const.SESSION_LEVEL,levelmap);
						return true;
					}
				}
			}
